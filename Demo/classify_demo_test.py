...
def get_model(input_ratio, feat_width, perm_width, neurons=32, dropout_rate=0.3, sss = StratifiedShuffleSplit(n_splits=1, random_state=0, test_size=0.20), perm_inputs=None, labels=None, feat_inputs=None, epochs=None, batch=None):
    cm = np.zeros([2,2], dtype=np.int64)
    model = create_dualInputLarge(input_ratio=.125, neurons=neurons, perm_width=perm_width, feat_width=feat_width)
    i = 0
...
def main():
   ...
   # model.summary()
    ...
    loaded_model = KerasClassifier(build_fn=get_model(.125, feat_width, perm_width, neurons=32, dropout_rate=0.3, sss = StratifiedShuffleSplit(n_splits=1, random_state=0, test_size=0.20), perm_inputs=perm_inputs, labels=labels, feat_inputs=feat_inputs, epochs=epochs, batch=batch), epochs=epochs, batch_size=batch, verbose=2)
